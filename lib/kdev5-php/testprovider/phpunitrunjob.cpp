...
void PhpUnitRunJob::processFinished(KJob* job)
{
    if (job->error() == 1) {
        ...
        Q_FOREACH(KDevelop::TestResult::TestCaseResult result, m_result.testCaseResults)
        {
...
{
    Q_ASSERT(m_outputJob);
    static QRegularExpression testResultLineExp = QRegularExpression("\\[([x\\s])\\]");
    for (int row = startRow; row <= endRow; ++row)
    {
        ...
        if (i > -1)
        {
            ...
            {
                Q_FOREACH(const QString& realCaseName, m_cases)
                {
