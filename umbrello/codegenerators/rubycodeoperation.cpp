    ...
    if (comment.isEmpty()) {
        if (getContentType() == CodeBlock::AutoGenerated) {
            ...
            foreach (UMLAttribute* currentAtt, parameters ) {
                comment += endLine + "* _" + currentAtt->getName() + "_ ";
                comment += (' ' + currentAtt->getDoc().replace( QRegExp("[\\n\\r]+[\\t ]*"),
                                                                        endLine + "  " ) );
            }
        ...
        foreach (UMLAttribute* currentAtt, parameters ) {
            ...
            if (commentedParams.contains(RubyCodeGenerator::cppToRubyName(currentAtt->getName())) == 0) {
                comment += (endLine + "@param _" + RubyCodeGenerator::cppToRubyName(currentAtt->getName()) + '_');
                if (currentAtt->getDoc().isEmpty()) {
