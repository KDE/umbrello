...
{
    return getUniqueTag(QLatin1String("hblock_tag"));
}
...
void HierarchicalCodeBlock::setAttributesOnNode (QXmlStreamWriter& writer)
{
    ...
    if (contentType() != CodeBlock::AutoGenerated)
    {
        ...
        writer.writeAttribute(QLatin1String("startText"), encodeText(getStartText(), endLine));
        writer.writeAttribute(QLatin1String("endText"), encodeText(getEndText(), endLine));
    }
}
...
void HierarchicalCodeBlock::setAttributesFromNode (QDomElement & root)
{
    ...
    {
        QString endLine = UMLApp::app()->commonPolicy()->getNewLineEndingChars();
        setStartText(decodeText(root.attribute(QLatin1String("startText")), endLine));
        setEndText(decodeText(root.attribute(QLatin1String("endText")), endLine));
    }
