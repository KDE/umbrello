...
/**
 * CodeWriter is the base class for all CodeWriters. It
 * provides the interface through which all Writers are invoked and
 * the all the basic functionality. The only thing it doesn't do is to
 * generate code =)
 *
 * If you want to implement a CodeWriter for some language follow
 * these steps:
 *
 * Create a class which inherits CodeWriter. This class can have
 * any name, I use names like CppWriter for the Cpp Writer,
 * JavaWriter for the Java Writer and so on, but you can use what
 * you want.
 *
...
public:
	...
	CodeWriter( QObject *parent = 0, const char *name = 0 );
	virtual ~CodeWriter();
 ...
 *
 * If you want to write a library containing Code Writers, you have
 * to implement the generators you want by subclassing CodeWriter
 * (see the doc for CodeWriter) and implement WriterFactory. Then
 ...
 * your generators what you want, so if you subclased CodeWriter
 * and created a class named "MyFirstCodeWriter", which outputs
 * Java code, then you should return a QString "MyFirstCodeWriter"
 ...
 * example, if name = "MyFirstCodeWriter", then you should create
