...
void TSWriter::writeClass(UMLClassifier *c)
{
    ...
    fileName = findFileName(c, QStringLiteral(".ts"));
    if (fileName.isEmpty())
    {
    ...
    QString str;
    str = getHeadingFile(QStringLiteral(".ts"));
    if (!str.isEmpty())
    {
    ...
    for (UMLPackage* conc :   includes) {
        QString headerName = findFileName(conc, QStringLiteral(".ts"));
 ...
 */
void TSWriter::writeAssociation(QString& classname, UMLAssociationList& assocList, QTextStream &js)
{
 ...
 * @param js          output stream for the JS file
 */
void TSWriter::writeOperations(QString classname, UMLOperationList *opList, QTextStream &js)
{
