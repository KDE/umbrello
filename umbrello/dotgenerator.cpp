...
QString DotGenerator::currentDotPath()
{
    ...
    QString appDir(QStringLiteral(qgetenv("ProgramFiles").constData()));
    QDir dir(appDir);
...
bool DotGenerator::createDotFile(UMLScene *scene, const QString &fileName, const QString &variant)
{
    ...
    foreach(AssociationWidget *assoc, scene->associationList()) {
        ...
        headLabel += QStringLiteral(QStringLiteral("  ")) + assoc->multiplicity(swapId ? Uml::RoleType::B : Uml::RoleType::A);
        tailLabel += QStringLiteral(QStringLiteral("  ")) + assoc->multiplicity(swapId ? Uml::RoleType::A : Uml::RoleType::B);

        QString edgeParameters;
        QStringList params;
        QString rkey = QStringLiteral(QStringLiteral("ranking::")) + key;
        ...
        QString vkey = QStringLiteral(QStringLiteral("visual::")) + key;
        if (m_edgeParameters.contains(vkey))
...
int DotGenerator::generatorVersion() const
{
    ...
    p.waitForFinished();
    QString out(QStringLiteral(p.readAllStandardError()));
