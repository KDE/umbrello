...
AssociationWidget& AssociationWidget::operator=(const AssociationWidget & Other)
{
    ...
    if (Other.m_pName) {
        m_pName = new FloatingTextWidget(umlScene());
    ...
    for (unsigned r = (unsigned)A; r <= (unsigned)B; r++) {
        ...
        if (rhs.m_pMulti) {
            lhs.m_pMulti = new FloatingTextWidget(umlScene());
            // DEPRECATED copy
        ...
        if (rhs.m_pRole) {
            lhs.m_pRole = new FloatingTextWidget(umlScene());
            // DEPRECATED copy
        ...
        if (rhs.m_pChangeWidget) {
            lhs.m_pChangeWidget = new FloatingTextWidget(umlScene());
            // DEPRECATED
...
void AssociationWidget::setName(const QString &strName)
{
    ...
    if(!m_pName) {
        ...
        newLabel = true;
        m_pName = new FloatingTextWidget(umlScene(), CalculateNameType(tr_Name), strName);
                                        ...
                                        FloatingTextWidget* &ft)
{
    ...
    if (ft == NULL) {
        ft = new FloatingTextWidget(umlScene(), tr, text);
        ft->setLink(this);
        ft->activate();
        setTextPosition(tr);
...
void AssociationWidget::setChangeWidget(const QString &strChangeWidget, Uml::Role_Type role)
{
    ...
    if(!m_role[role].m_pChangeWidget) {
        ...
        newLabel = true;
        m_role[role].m_pChangeWidget = new FloatingTextWidget(umlScene(), tr, strChangeWidget);
...
void AssociationWidget::mergeAssociationDataIntoUMLRepresentation()
{
    ...
    if (text)
        umlObject()->setName(text->getText());
                                     ...
                                     const MessageWidgetList* pMessages )
{
    ...
    while( !element.isNull() ) {
        ...
        if( tag == "linepath" ) {
            ...
            FloatingTextWidget *ft = new FloatingTextWidget(umlScene(), role, "", Uml::id_Reserved);
