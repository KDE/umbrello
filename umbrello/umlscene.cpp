 ...
 */
void UMLScene::setupNewWidget(UMLRectWidget *w)
{
	if(w->scene() != this) {
		addItem(w);
	}
...
void UMLScene::slotObjectCreated(UMLObject* o)
{
    ...
    UMLRectWidget* newWidget = Widget_Factory::createWidget(this, o);
    ...
    case ot_Category:
        ...
        foreach(UMLRectWidget* w,  m_WidgetList) {
            if (w != newWidget) {
...
void UMLScene::slotObjectRemoved(UMLObject * o)
{
    ...
    foreach(UMLRectWidget* obj, m_WidgetList) {
        if (obj->id() != id)
            continue;
        removeWidget(obj);
        break;
 ...
 */
UMLRectWidget *UMLScene::getWidgetAt(const QPointF& p)
{
    int relativeSize = 10000;  // start with an arbitrary large number
    UMLRectWidget  *retObj = NULL;
...
bool UMLScene::widgetOnDiagram(Uml::IDType id)
{
    ...
    foreach(UMLRectWidget *obj , m_MessageList) {
        if (id == obj->id())
            return true;
    }
...
// search both our UMLRectWidget AND MessageWidget lists
 ...
 */
UMLRectWidget * UMLScene::findWidget(Uml::IDType id)
{
    foreach(UMLRectWidget* obj, m_WidgetList) {
 ...
 *
 * @param pWidgetA  Pointer to the UMLRectWidget of role A.
 * @param pWidgetB  Pointer to the UMLRectWidget of role B.
 ...
 * @return Returns the widget found, returns 0 if no widget found.
 */
AssociationWidget * UMLScene::findAssocWidget(UMLRectWidget *pWidgetA,
 ...
 */
void UMLScene::removeWidget(UMLRectWidget * o)
{
    if (!o)
        return;
 ...
 */
void UMLScene::setSelected(UMLRectWidget * w, QGraphicsSceneMouseEvent * me)
{
    Q_UNUSED(me);
...
UMLWidgetList UMLScene::selectedWidgets() const
{
    ...
    foreach(QGraphicsItem *item, selectedItems()) {
        UMLRectWidget *wid = dynamic_cast<UMLRectWidget*>(item);
        if(wid) {
            list << wid;
        }
    }
 ...
 */
UMLRectWidget* UMLScene::getFirstMultiSelectedWidget() const
{
...
void UMLScene::moveSelectedBy(qreal dX, qreal dY)
{
    ...
    foreach(UMLRectWidget *w, selectedWidgets()) {
        w->moveBy(dX, dY);
    }
}
...
{
    foreach(UMLRectWidget* temp, selectedWidgets()) {
...
/**
 * Calls setSelected on the given UMLRectWidget and enters
 * it into the m_SelectedList while making sure it is
 * there only once.
 */
void UMLScene::makeSelected(UMLRectWidget * uw)
{
    if (uw) {
        uw->setSelected(true);
    }
...
void  UMLScene::getDiagram(const QRectF &area, QPainter & painter)
{
    ...
    foreach(UMLRectWidget* widget , selected) {
...
void UMLScene::activate()
{
    ...
    foreach(UMLRectWidget* obj , m_WidgetList) {
        //If this UMLRectWidget is already activated or is a MessageWidget then skip it
 ...
 */
bool UMLScene::addWidget(UMLRectWidget * pWidget , bool isPasteOperation)
{
    if (!pWidget) {
        return false;
    }
...
bool UMLScene::addAssociation(AssociationWidget* pAssoc , bool isPasteOperation)
{
    ...
    UMLRectWidget * pWidgetA = findWidget(pAssoc->getWidgetID(A));
    UMLRectWidget * pWidgetB = findWidget(pAssoc->getWidgetID(B));
...
void UMLScene::removeAssociations(UMLRectWidget* Widget)
{
...
void UMLScene::removeAllWidgets()
{
    ...
    foreach(UMLRectWidget *temp, m_WidgetList) {
 ...
 *  Calls the same method in the DocWindow.
 */
void UMLScene::showDocumentation(UMLRectWidget * widget, bool overwrite)
{
...
void UMLScene::updateContainment(UMLCanvasObject *self)
{
    ...
    UMLRectWidget *selfWidget = NULL, *newParentWidget = NULL;
    ...
    foreach(AssociationWidget* a, m_AssociationList) {
        ...
        UMLRectWidget *wB = a->getWidget(B);
        UMLObject *roleBObj = wB->umlObject();
        if (roleBObj != self)
            continue;
        UMLRectWidget *wA = a->getWidget(A);
...
/**
 * Creates automatically any Associations that the given @ref UMLRectWidget
 * may have on any diagram.  This method is used when you just add the UMLRectWidget
 * to a diagram.
 */
void UMLScene::createAutoAssociations(UMLRectWidget * widget)
{
    if (widget == NULL ||
        (m_Type != Uml::dt_Class &&
 ...
 */
void UMLScene::createAutoAttributeAssociations(UMLRectWidget *widget)
{
...
/**
 * Create an association with the attribute attr associated with the UMLRectWidget
...
void UMLScene::createAutoAttributeAssociation(UMLClassifier *type, UMLAttribute *attr,
                                              UMLRectWidget *widget /*, UMLClassifier * klass*/)
{
    if (type == NULL) {
    ...
    UMLRectWidget *w = findWidget(type->getID());
    ...
    if (type->getBaseType() == ot_Datatype) {
        ...
        if (dt->isReference()) {
            ...
            UMLRectWidget *w = c ? findWidget(c->getID()) : 0;
...
void UMLScene::createAutoConstraintAssociations(UMLRectWidget *widget)
{
...
void UMLScene::createAutoConstraintAssociation(UMLEntity* refEntity, UMLForeignKeyConstraint* fkConstraint, UMLRectWidget* widget)
{
    ...
    UMLRectWidget *w = findWidget(refEntity->getID());
    AssociationWidget *aw = NULL;
...
{
    foreach(UMLRectWidget* pWidget , m_WidgetList) {
...
{
    UMLRectWidget * pWA = 0, * pWB = 0;
    //check messages
    foreach(UMLRectWidget *pTemp , selectedWidgets()) {
...
bool UMLScene::checkUniqueSelection()
{
    ...
    UMLRectWidget * pTemp = (UMLRectWidget *) selectedWidgets().first();
...
void UMLScene::saveToXMI(QDomDocument & qDoc, QDomElement & qElement)
{
    ...
    foreach(UMLRectWidget *widget , m_WidgetList) {
    ...
    foreach(UMLRectWidget* widget , m_MessageList) {
 ...
 */
UMLRectWidget* UMLScene::loadWidgetFromXMI(QDomElement& widgetElement)
{
    ...
    UMLRectWidget* widget = Widget_Factory::makeWidgetFromXMI(tag, idstr, this);
    ...
    for (QDomNode node = qElement.firstChild(); !node.isNull(); node = node.nextSibling()) {
        ...
        if (o == NULL) {
            ...
            case Uml::ot_Association: {
                ...
                UMLRectWidget *wA = findWidget(objA->getID());
                UMLRectWidget *wB = findWidget(objB->getID());
                if (wA != NULL && wB != NULL) {
 ...
 */
bool UMLScene::hasWidgetSmallerX(const UMLRectWidget* widget1, const UMLRectWidget* widget2)
{
 ...
 * @param widget2 The widget to compare with.
 */
bool UMLScene::hasWidgetSmallerY(const UMLRectWidget* widget1, const UMLRectWidget* widget2)
{
...
{
    QVector<UMLRectWidget*> widgetVector;
    ...
    for (QVector<UMLRectWidget*>::iterator it = widgetVector.begin(); it != widgetVector.end(); ++it) {
