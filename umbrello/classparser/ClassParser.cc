...
void CClassParser::fillInParsedStruct( CParsedContainer *aContainer )
{
  ...
  if( !lexemStack.isEmpty() && lexemStack.top()->type == IDEN )
  ...
  getNextLexem( );
  while( lexem == IDEN || lexem == ',' ){  // here we're parsing <name> <name...>
    if( lexem == IDEN )
      ps.append( getText( ) );
    getNextLexem( );
  }
  ...
  //  if( lexem == IDEN )                --- commented out by Daniel
...
void CClassParser::parseNamespace( CParsedScopeContainer * scope )
{
  ...
  // retrieve it
  CParsedScopeContainer *ns2 = scope->getScopeByName(ns->name.latin1());
  if (ns2)
  {
    delete ns;
    ns = ns2;
...
void CClassParser::fillInParsedVariableHead( CParsedAttribute *anAttr )
{
  ...
  if( lexemStack.top()->type == ')' )
  {
    ...
    while( !exit )
    {
      ...
      {
        thisIsVarName= (!lexemStack.isEmpty() && aLexem->type == IDEN &&
                       (lexemStack.top()->type==IDEN ||
                        lexemStack.top()->type=='*' ||
                        lexemStack.top()->type=='}' ||
                        lexemStack.top()->type=='&'));
      }
  ...
  {
    while( !lexemStack.isEmpty() && lexemStack.top()->type != IDEN )
      delete lexemStack.pop();
  }
...
void CClassParser::fillInParsedVariable( CParsedAttribute *anAttr )
{
  ...
  if( !type.isEmpty() ) {
	  QString catType = type + anAttr->type;
	  anAttr->setType( catType.latin1() );
  }
...
void CClassParser::fillInMultipleVariable( CParsedContainer *aContainer )
{
  ...
  while( !lexemStack.isEmpty() && !exit )
  {
    ...
    {
      // Add pointer stuff
      while(  lexemStack.top()->type != IDEN && lexemStack.top()->type != ',' )
      {
        aLexem = lexemStack.pop();
      ...
      exit = (lexemStack.isEmpty() || lexemStack.top()->type == IDEN );
...
void CClassParser::parseMethodImpl(bool isOperator,CParsedContainer *scope)
{
  ...
  lexemStack.push( new CParsedLexem( IDEN, name.latin1() ) );
  ...
  if( lexem != ';' )
  {
    ...
    aClass = store.getClassByName( className.latin1() );
    if( aClass != NULL)
    {
...
// EO
  	{
  	  ...
  	  if (scope->path().latin1())        // Don't produce bad class names
  	    path = scope->path() + "." + path;

      kdDebug() << "scope path is " << path << endl;
      aClass = store.getClassByName( path.latin1() );
      if( aClass != NULL)
      {
        pm = aClass->getMethod( aMethod );
        if( pm != NULL )
...
CParsedClass *CClassParser::parseClassHeader()
{
  ...
  while( lexem != 0 && ( lexem == IDEN || lexem == CLCL ) )
  {
    if( lexem == CLCL )
      foundCLCL = true;
  ...
  switch( lexem )
  {
    ...
    case CPCLASS:
      ...
      if( childClass != NULL )
      {
        ...
        if( store.hasClass( childClass->path().latin1() ) ) {
  	      CParsedClass *	parsedClassRef = store.getClassByName( childClass->path().latin1() );
    ...
    case CPCLASS:
      ...
      if( aClass != NULL )
      {
        ...
        else if( !scope->path().isEmpty() )
        {
          ...
          parentClass = store.getClassByName( scope->path().latin1() );

          // If we didn't find a parent class, try to find a namespace.
          if( parentClass == NULL )
          {
            parentScope = store.getScopeByName( scope->path().latin1() );
        ...
        if( store.hasClass( aClass->path().latin1() ) ) {
  	      CParsedClass *	parsedClassRef = store.getClassByName( aClass->path().latin1() );
        ...
        {
          // Get the parent class;
          parentClass = store.getClassByName( scopePath.latin1() );

          // If we didn't find a parent class, try to find a namespace.
          if( parentClass == NULL )
          {
            parentScope = store.getScopeByName( scopePath.latin1() );
