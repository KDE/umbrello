...
void CParsedClassContainer::addClass( CParsedClass *aClass )
{
  ...
  REQUIRE( "Unique class <"+aClass->path()+">", !hasClass( useFullPath ? aClass->path().latin1() : aClass->name.latin1() ) );
...
QStrList *CParsedClassContainer::getSortedClassNameList(bool bUseFullPath)
{
  ...
  {
     if (bUseFullPath)
       ret_val->append(classIterator.currentKey().latin1());
     else
     {
       QString path=act->path();
    //   path.replace(QRegExp("."),"::");
       ret_val->append(path.latin1());
     }
  };
