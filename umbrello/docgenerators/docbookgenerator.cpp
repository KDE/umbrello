...
bool DocbookGenerator::generateDocbookForProject()
{
  ...
  return generateDocbookForProjectInto(url);
}

bool DocbookGenerator::generateDocbookForProjectInto(const KURL& destDir)
{
  UMLApp* app = UMLApp::app();
  UMLDoc* umlDoc = app->getDocument();
...
//   QCString utf8 = xmi.utf8();
//   const char *c(utf8);
  doc = xmlParseFile((const char*)(tmpfile.name().utf8()));
//   doc = xmlParseDoc((const xmlChar*)c);
  ...
  KIO::copy(tmpDocBook.file()->name(),url);
