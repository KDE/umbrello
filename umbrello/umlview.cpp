	...
	if(m_bCreateObject) {
		...
		if(type == ot_Actor) {
			...
			if(m_pData -> m_Type == dt_Class) {
				ConceptWidget * c = new ConceptWidget(this, o);
				temp = (UMLWidget*)c;
			} else {
		...
		temp -> setX( m_Pos.x() );
		temp -> setY( y );
		temp -> moveEvent( 0 );//needed for ObjectWidget to set seq. line properly
		temp -> setVisible( true );
		temp -> setActivated();
		temp -> setFont( m_pData -> getFont() );
		temp->slotColorChanged( m_pData->getID() );
...
void UMLView::contentsDragEnterEvent(QDragEnterEvent *e) {
	...
	if(status) {
		...
		if((m_pData->m_Type == dt_Sequence || m_pData->m_Type == dt_Class || m_pData->m_Type == dt_Collaboration)
		   && (ot != ot_Concept) ) {
				status = false;
		}
...
void  UMLView::getDiagram(const QRect &rect, QPixmap & diagram) {
	...
	canvas()->drawArea( QRect( 0, 0, pixmap.width(), pixmap.height() ), &painter );
	painter.end();
