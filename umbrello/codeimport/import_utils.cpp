 ...
 */
bool parseStackTraceLine(const QString &s, QString &sequence, QString &package, QString &method)
{
    QString identifier;
    QString module;
...
/**
 * Import stack trace from a string list.
 *
 * @param lines String list with stack trace
 * @param scene The diagram to import the sequences into.
 * @return true Import was successful.
 * @return false Import failed.
 */
bool importStackTrace(const QStringList &lines, UMLScene *scene)
{
    ...
    foreach(const QString &line, l) {
        ...
        if (!parseStackTraceLine(line, stackframe, package, method))
            continue;
...
/**
 * Import stack trace from clipboard
 *
 ...
 * @return false Import failed.
 */
bool importStackTrace(const QMimeData* mimeData, UMLScene *scene)
{
    ...
    doc->beginPaste();
    bool result = importStackTrace(lines, scene);
    doc->endPaste();
    return result;
}

/**
 * Import stack trace from file
 *
...
bool importStackTrace(const QString &fileName, UMLScene *scene)
{
    ...
    return importStackTrace(lines, scene);
}
